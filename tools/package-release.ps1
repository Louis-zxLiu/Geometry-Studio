...
function Write-PortableReadme {
    ...
    $contentLines = @(
        "Geometry Studio portable release",
        "",
        "How to run:",
        "1. Extract the whole zip into a writable directory, for example Desktop or D:\Apps\GeometryStudio.",
        "2. Double-click GeometryStudio.exe.",
        "3. On first launch, the app extracts resources/runtime/runtime.7z into runtime/ automatically.",
        "",
        ...
        "",
        "Notes:",
        "- Do not run GeometryStudio.exe directly inside the zip. Extract the package first.",
        "- Do not distribute GeometryStudio.exe alone. Keep the resources/ directory with it.",
        "- This package does not require Go, Node.js, Wails, npm, or a system Python install.",
        "- Windows still needs Microsoft Edge WebView2 Runtime. Most Windows 10/11 machines already include it.",
        "- AI features need network access and valid model URL, KEY, and MODEL settings.",
        "- Image-based geometry solving needs a multimodal model that supports image input.",
        "- If you see 'write |1: The pipe has been ended.', the geometry worker exited early. Common causes are incomplete extraction, missing resources/runtime, invalid AI settings, provider errors, or a model that does not support the current input."
    )
